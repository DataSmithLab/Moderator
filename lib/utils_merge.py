...
from lib.utils_analysis import *
...
def task_vectors_merge(merge_function, tv_flat_checks):
...
def tv_keys_split(input_list, n):
    list_length = len(input_list)
    elements_per_partition = list_length // n
    partitions = []

    for i in range(0, list_length, elements_per_partition):
        partition = input_list[i:i + elements_per_partition]
        partitions.append(partition)

    return partitions

def merge_main(merge_func, tv_filenames):
    device = torch.device('cpu') # move merge operations to cpu due to huge memory consumption
    
    example_state_dict = np.load(tv_filenames[0], allow_pickle=True).item()
    tv_keys = list(example_state_dict.keys())
    del example_state_dict
    #print("tv key extracted")
    
    merged_state_dict = {}
    split_tv_key_partitions = tv_keys_split(tv_keys, len(tv_filenames)*3)
    for tv_key_partition in split_tv_key_partitions:
        print(tv_key_partition)
        key_tv_state_dicts = []
        for tv_filename in tv_filenames:
            tv = np.load(tv_filename, allow_pickle=True).item()
            key_tv = {}
            for tv_key in tv_key_partition:
                key_tv[tv_key]=tv[tv_key]
            del tv
            key_tv_state_dicts.append(key_tv)
        #print("tv load complete")
        remove_keys = []
        flat_ft = torch.vstack(
            [state_dict_to_vector(check, remove_keys) for check in key_tv_state_dicts]
        )
        #print("state dicts flat complete")
        reference_state_dict = key_tv_state_dicts[0]
        #print("reference", reference_state_dict.keys())
        del key_tv_state_dicts

        merged_tv = task_vectors_merge(merge_func, flat_ft)
        #print(" tv merged complete")

        merged_checkpoint = vector_to_state_dict(
            merged_tv, reference_state_dict, remove_keys=remove_keys
        )
        del merged_tv, reference_state_dict
        print("task vector merge complete")
        for tv_key in tv_key_partition:
            merged_state_dict[tv_key]=merged_checkpoint[tv_key]
    merged_task_vector = TaskVector(vector=merged_checkpoint)
    return merged_task_vector
    #np.save(merged_tv_filename, merged_state_dict)
    #torch.save(merged_state_dict, merged_tv_filename)